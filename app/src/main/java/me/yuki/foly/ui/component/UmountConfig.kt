package me.yuki.foly.ui.component
...
object UmountConfigManager {
    ...
    fun loadConfig(context: Context): UmountConfig {
        return try {
            val shell = me.yuki.foly.util.getRootShell()

            val enabledFlag = try {
                val result = shell.newJob().add("test -f ${me.yuki.foly.APApplication.UMOUNT_SERVICE_FILE}").exec()
                result.isSuccess
    ...
    fun saveConfig(context: Context, config: UmountConfig): Boolean {
        ...
        return try {
            ...
            me.yuki.foly.util.setUmountServiceEnabled(config.enabled)
