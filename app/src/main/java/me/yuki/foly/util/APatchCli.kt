...
import me.yuki.foly.APApplication.Companion.SUPERCMD
...
fun createRootShell(globalMnt: Boolean = false): Shell {
    ...
    // Try multiple SU paths for different root solutions (Magisk, KernelSU, APatch)
    val suPaths = listOf(
        "/system/bin/su",
        "/system/xbin/su",
        "/sbin/su",
    ...
    // First, try to find a working SU binary
    for (suPath in suPaths) {
        try {
            val testShell = Shell.Builder.create().build("sh")
            val result = testShell.newJob().add("test -x $suPath").exec()
            testShell.close()
            if (result.isSuccess) {
                ...
                return try {
                    ...
                    Log.w(TAG, "SU at $suPath failed: ${e.message}")
                    continue
                }
            }
        } catch (e: Throwable) {
...
suspend fun listModules(): String = withContext(Dispatchers.IO) {
    ...
    withNewRootShell {
        newJob().add("cp /data/user/*/me.yuki.foly/patch/ori.img /data/adb/ap/ && rm /data/user/*/me.yuki.foly/patch/ori.img")
...
fun installModule(
    ...
    val inputStream = try {
        ...
        Handler(Looper.getMainLooper()).post {
            me.yuki.foly.util.ui.showToast(apApp, permissionMessage)
        }
