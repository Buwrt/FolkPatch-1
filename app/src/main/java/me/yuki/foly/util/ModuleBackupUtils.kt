...
object ModuleBackupUtils {
    ...
    suspend fun autoBackupModule(context: Context, file: File, originalFileName: String?, subDir: String): String? {
        return withContext(Dispatchers.IO) {
            ...
            val webdavJob = async {
                if (me.yuki.foly.ui.theme.BackupConfig.isBackupEnabled) {
                     try {
                         val basePath = me.yuki.foly.ui.theme.BackupConfig.webdavPath
                         ...
                         val webDavResult = WebDavUtils.uploadFile(
                            me.yuki.foly.ui.theme.BackupConfig.webdavUrl,
                            me.yuki.foly.ui.theme.BackupConfig.webdavUsername,
                            me.yuki.foly.ui.theme.BackupConfig.webdavPassword,
                            file,
                            cleanSubDir,
                            originalFileName
                         )
            ...
            val localJob = async {
                if (APApplication.sharedPreferences.getBoolean("auto_backup_module", false)) {
                    try {
                        val baseBackupDir = File(getSafeDownloadsDir(me.yuki.foly.apApp), "FolkPatch/ModuleBackups")
